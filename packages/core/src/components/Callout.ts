...
export default async (block: ExtractBlock<'callout'>) => {
  ...
  return `<div style="display: flex; padding: 0.5rem; gap: 0.25rem;">${iconHTML}<div>${text}<div>${(await Fragment(block.children)).html}</div></div></div>`
}
